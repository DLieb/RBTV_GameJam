...
public class PowerUp : MonoBehaviour
{
	...
	public AudioClip[] CustomPowerSounds;
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        if (other.gameObject.tag == "Player")
        {
            ...
            if (!temp.hasPowerUp)
            {
				other.gameObject.GetComponent<PowerUPTransform>().transformPower(this.Attack, this.PowerChar, this.PowerTime, this.CustomPowerSounds, this.AmbientSound);
                Destroy(this.gameObject);
            }
        }
    }
